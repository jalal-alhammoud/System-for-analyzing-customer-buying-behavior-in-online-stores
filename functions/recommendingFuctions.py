...
#تابع يقوم بحساب درجة التطابق بين مستخدمين
#   تحسب درجة التطابق بناء على الاهتمامات المشتركةالفرق العمري اشكال التفاعل معدل التفاعل الموقع
...
def find_similar_users(target_user_id, users_with_interests_df, reviews_df, interactions_df, n_recommendations=5):
    ...
    المعاملات:
        target_user_id: معرف المستخدم المستهدف
        users_with_interests_df: DataFrame يحتوي على بيانات المستخدمين مع اهتماماتهم (نتيجة التابع السابق)
        reviews_df: DataFrame المراجعات
        interactions_df: DataFrame التفاعلات
        n_recommendations: عدد المستخدمين المشابهين المطلوبين
        
    المخرجات:
        DataFrame يحتوي على المستخدمين الأكثر تشابهًا مع درجات التشابه
    """
    ...
    # إنشاء ميزات للتشابه
    def prepare_features(user):
        # الميزات العددية
        features = {
            'age': user['age'],
            'avg_preferred_price': user.get('avg_preferred_price', 0),
            'n_reviews': reviews_df[reviews_df['user_id'] == user['user_id']].shape[0],
            'n_interactions': interactions_df[interactions_df['user_id'] == user['user_id']].shape[0],
        }
        
        # ميزات الاهتمامات (تشفير one-hot للمجموعات)
        for cat in ['interests_categories', 'preferred_brands', 'common_interaction_types']:
            for item in user.get(cat, []):
                features[f"{cat}_{item}"] = 1
        
        return features
    
    # إعداد ميزات للمستخدم المستهدف وجميع المستخدمين
    target_features = prepare_features(target_user)
    
    all_users_features = []
    user_ids = []
    
    for _, user in users_with_interests_df.iterrows():
        if user['user_id'] == target_user_id:
            continue  # تخطي المستخدم المستهدف
        
        user_features = prepare_features(user)
        all_users_features.append(user_features)
        user_ids.append(user['user_id'])
    
    # تحويل إلى DataFrame
    features_df = pd.DataFrame(all_users_features).fillna(0)
    ...
    features_df = features_df[target_df.columns]
    
    # تطبيع البيانات
    scaler = MinMaxScaler()
    all_data = pd.concat([target_df, features_df])
    scaled_data = scaler.fit_transform(all_data)
    
    target_scaled = scaled_data[0].reshape(1, -1)
    others_scaled = scaled_data[1:]
...
def get_products_from_product(product_ids):
    ...
    for pro_id in product_ids:
            ...
            if 'description' in df_pro.columns:
                rec_description= recommend_products_by_description(df_pro, user_description=df_pro.loc[df_pro['product_id'] == pro_id,'description'].values[0],description_column="description",top_n=2 )
...
def suggest_for_user(user_id):
    ...
    #فحص لمعرفة إذا كان المستخدم قد تفاعل سابقاً مع الموقع
    if (user_id in df_review['user_id'].values) and (user_id in df_inter['user_id'].values) :
        user_product = get_user_prefered_product(df_review, df_inter, user_id)
        ...
        for pro_id in product_ids:
            ...
            if context is None:
                input_data = {
                        ...
                        'price': product_feature['price'],
                        'location_user': user_feature['location_user'],
                        'review_text': review_text,
                        'category': product_feature['category'],
                ...
                input_data = {
                        ...
                        'location_con': context['location_con']
                    }
            deep_predicat= 5 * deeplearn.predict_rating(input_data)
            # deep_predicat= 5 * 0.5
            # print("svd_predicat")
            # print(svd_predicat)
            # print("knn_predicat")
            # print(knn_predicat)
            # print("light_predicat")
            # print(light_predicat)
            # print("deep_predicat")
            # print(deep_predicat)
...
# تابع يقوم بتجميع المستخدمين حسب الصفات الخاصة بكل مستخدم وحسب التفاعل مع المنتجات
# تكون نتيجة التجميع كالتالي
# {
#         'features_clusters': features_clusters_df,   التجميع حسب الصفات
#         'interactions_clusters': interactions_clusters_df, التجميع حسب التفاعل
#         'combined_clusters': combined_clusters_df, التحميع حسب الصفات والتفاعل
#         'cluster_centers': cluster_centers_df  مراكز التجميع
#     }
def cluster_all_users(users_df, reviews_df, interactions_df, n_clusters=5):
    ...
    # إذا كان هناك اختلاف في الأحجام، نأخذ التقاطع بين المستخدمين
    common_users = users_df['user_id'].isin(interaction_data['user_id'])
    features_transformed = features_transformed[common_users]
    interaction_scaled = interaction_scaled[common_users]
    ...
    result = {
        ...
        "cluster_centers": {
            ...
            "interactions": KMeans(n_clusters=n_clusters).fit(interaction_scaled).cluster_centers_
        },
        "user_mapping": users_df[common_users]['user_id'].to_list(),
    ...
    for idx, user_id in enumerate(users_df[common_users]['user_id']):
        result["feature_clusters"][features_clusters[idx]].append(user_id)
        
    for idx, user_id in enumerate(interaction_data[common_users]['user_id']):
